...
func dither(image: Image) -> Image:
	...
	for x in range(image.get_width()):
		for y in range(image.get_height()):
			...
			adjust_pixel(working_image, x + 1, y + 0, error * (1.0 / 8.0))
			adjust_pixel(working_image, x + 2, y + 0, error * (1.0 / 8.0))
			adjust_pixel(working_image, x - 1, y + 1, error * (1.0 / 8.0))
			adjust_pixel(working_image, x + 0, y + 1, error * (1.0 / 8.0))
			adjust_pixel(working_image, x + 1, y + 1, error * (1.0 / 8.0))
			adjust_pixel(working_image, x + 0, y + 2, error * (1.0 / 8.0))
