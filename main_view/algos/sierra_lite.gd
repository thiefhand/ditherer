...
func dither(image: Image) -> Image:
	...
	for x in range(image.get_width()):
		for y in range(image.get_height()):
			...
			adjust_pixel(working_image, x + 1, y + 0, error * (2.0 / 4.0))
			adjust_pixel(working_image, x - 1, y + 1, error * (1.0 / 4.0))
			adjust_pixel(working_image, x + 0, y + 1, error * (1.0 / 4.0))
